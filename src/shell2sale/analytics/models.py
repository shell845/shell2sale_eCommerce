...
from django.contrib.sessions.models import Session # handle session
from django.db import models
from django.db.models.signals import pre_save, post_save # handle signal

from accounts.signals import user_logged_in
from .signals import object_viewed_signal # import signal
...
class ObjectViewedQuerySet(models.query.QuerySet):
    def by_model(self, model_class, model_queryset=False):
        ...
        qs = self.filter(content_type=c_type).order_by('timestamp')[:12]
        print('*** qs ', qs)
        print('*** c_type', c_type)
        if model_queryset:
...
class ObjectViewed(models.Model):
    ...
    class Meta:
        ordering = ['-timestamp'] # show most recent saved first
        verbose_name = 'Object viewed'
        verbose_name_plural = 'Objects viewed'


def object_viewed_receiver(sender, instance, request, *args, **kwargs): # sender receiver
...
def post_save_session_receiver(sender, instance, created, *args, **kwargs):
    if created:
        ...
        for i in qs:
            i.end_session()
    if not instance.active and not instance.ended: # admin can inactive the user and delete user session
...
def post_save_user_changed_receiver(sender, instance, created, *args, **kwargs):
    if not created:
        if instance.is_active == False:
            qs = UserSession.objects.filter(user=instance.user, ended=False, active=False) # bug pending to fix: AttributeError: 'User' object has no attribute 'user'. Original: user=instance.user
            for i in qs:
                i.end_session()
